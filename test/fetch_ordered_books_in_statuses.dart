...
import 'package:szufladka/custom_code/actions/index.dart';
...
void main() {
  ...
  setUp(() async {
    ...
    OrderStatus.values.forEach((status) async {
      await firestore.collection('orders').add(createOrdersRecordData(
  ...
  test("should return books with order with status", () async {
    //Act
    final result = await fetchOrderedBooksInStatuses([OrderStatus.Pending]);

    //Assert
    expect(result.length, 1);
  });

  test("should return books with order by multiple statuses", () async {
    //Act
    final result = await fetchOrderedBooksInStatuses(
        [OrderStatus.Pending, OrderStatus.Archived]);
